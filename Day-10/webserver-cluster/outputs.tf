...
# for expression in output — produces a map of alarm name → threshold
output "alarm_thresholds" {
  description = "CPU thresholds used for scaling decisions"
  value = var.enable_autoscaling ? {
    scale_out = local.scale_out_threshold
    scale_in  = local.scale_in_threshold
  } : {}
}