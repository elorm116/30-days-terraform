...
# -----------------------------
# LOCALS — centralise conditional logic
# -----------------------------

# Instead of scattering ternary operators through resource arguments,
# centralise decisions in locals. Clean and readable.
locals {
  # Scale out when CPU > 70% in production, 90% in dev
  # (dev doesn't need aggressive scaling)
  scale_out_threshold = var.environment == "production" ? 70 : 90

  # Scale in when CPU < 30% in production, 20% in dev
  scale_in_threshold = var.environment == "production" ? 30 : 20
}
...
# count = 1 → policy created
# count = 0 → policy skipped entirely
# This lets callers disable autoscaling for dev to save costs
...
# These alarms only make sense if autoscaling is enabled
# so they're also gated by the same variable
